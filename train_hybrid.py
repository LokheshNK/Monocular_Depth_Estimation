...
train_hybrid.py
─────────────────────────────────────────────────────────────
Train the Hybrid CNN + ViT depth model.

Model architecture lives entirely in:
    hybrid_model.py  →  HybridDepthModel
        └── cnn_model.py   (CNNEncoder, CNNDecoder)
        └── vit_model.py   (TransformerBlock used inside ViTBottleneck)

This file only handles:
    - Dataset loading + augmentation
    - Loss functions
    - Training / validation loop
    - Saving → best_hybrid_model.pth
...
# ══════════════════════════════════════════════════════════
# IMPORT MODEL FROM hybrid_model.py
# ══════════════════════════════════════════════════════════
# HybridDepthModel internally uses:
#   CNNEncoder  + CNNDecoder  from cnn_model.py
#   TransformerBlock          from vit_model.py  (via ViTBottleneck)
...
# Just add more KITTI sequence folders and they are picked up.
# Preview: python find_sequences.py
SEQUENCE_PAIRS = auto_find_sequences(BASE_DIR, verbose=True)

IMG_SIZE   = (128, 416)   # (H, W) — must match vit_spatial in HybridDepthModel
BATCH_SIZE = 4
EPOCHS     = 40
MAX_DEPTH  = 80.0
LR_ENC     = 5e-6         # VGG16 pretrained encoder — very gentle
LR_DEC     = 5e-5         # decoder + ViT bottleneck — normal
VAL_SPLIT  = 0.1
...
class KITTIDepthDataset(Dataset):
    ...
    Augmentation: horizontal flip, brightness/contrast, random crop.
    All augmentations are applied jointly to RGB and depth.
    """
...
class BerHuLoss(nn.Module):
    """Reverse Huber: L1 for small errors, L2 for large. Sparse-depth safe."""
    ...
    def forward(self, pred, gt):
        ...
        d = torch.abs(p - g)
        c = self.t * d.max().detach()
        return torch.where(d <= c, d, (d**2 + c**2) / (2*c + 1e-8)).mean()


class SSIMLoss(nn.Module):
    """Structural Similarity loss — reduces blurriness."""
...
class EdgeGradientLoss(nn.Module):
    """Sobel gradient matching — forces sharp depth edges."""
    def __init__(self):
        super().__init__()
...
# ══════════════════════════════════════════════════════════
# MODEL — object created from hybrid_model.py
# ══════════════════════════════════════════════════════════

model = HybridDepthModel(
    max_depth      = MAX_DEPTH,
    pretrained_cnn = True,       # use ImageNet VGG16 weights
    vit_embed_dim  = 256,        # ViT token dimension inside bottleneck
    vit_num_heads  = 8,          # attention heads
    vit_num_layers = 4,          # transformer depth
    vit_spatial    = (4, 13),    # enc5 spatial size for 128×416 input
).to(device)

criterion = CombinedLoss()

# Separate LRs: pretrained CNN encoder gets much smaller LR
optimizer = torch.optim.AdamW([
    {'params': model.cnn_encoder.block3.parameters(), 'lr': LR_ENC},
    {'params': model.cnn_encoder.block4.parameters(), 'lr': LR_ENC},
    {'params': model.cnn_encoder.block5.parameters(), 'lr': LR_ENC},
    {'params': model.vit_bottleneck.parameters(),     'lr': LR_DEC},
    {'params': model.cnn_decoder.parameters(),        'lr': LR_DEC},
], weight_decay=1e-4)
...
print(f"\n{'='*60}")
print(f"  HYBRID MODEL  |  {EPOCHS} epochs  |  {device}")
...
axes[0].set_title('Hybrid — Total Loss'); axes[0].set_xlabel('Epoch')
...
plt.tight_layout()
plt.savefig('hybrid_loss.png', dpi=150)
plt.show()
print("Saved → hybrid_loss.png")
...
print(f"\n{'='*60}")
print(f"  Hybrid — Final Validation Metrics")
print(f"{'='*60}")
