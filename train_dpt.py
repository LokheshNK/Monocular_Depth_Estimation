...
train_dpt.py
─────────────────────────────────────────────────────────────
Train the DPT (Dense Prediction Transformer) depth model.

Model architecture lives entirely in:
    depth_prediction_transformer.py  →  DepthPredictionTransformer
        └── vit_model.py  (PatchEmbedding, TransformerBlock, ViTBackbone)

This file only handles:
    - Dataset loading + augmentation
    - Loss functions
    - Training / validation loop
    - Saving → best_dpt_model.pth

Run:
    python train_dpt.py

Memory tip: if you get OOM, reduce BATCH_SIZE to 2.
─────────────────────────────────────────────────────────────
"""
...
# ══════════════════════════════════════════════════════════
# IMPORT MODEL FROM depth_prediction_transformer.py
# ══════════════════════════════════════════════════════════
# DepthPredictionTransformer internally uses:
#   PatchEmbedding, MultiHeadSelfAttention, TransformerBlock  from vit_model.py
#   ViTBackbone, Reassemble, FusionBlock, DPTDepthHead        defined in dpt file
# No architecture is defined here — just instantiated.
...
# ── Auto-discover ALL sequences in your dataset folder ────
# Scans raw_rgb/ and data_depth_annotated/train/ automatically.
# No need to manually list sequences — just add more folders
# and they will be picked up on the next run.
#
# To preview what will be found before training:
#     python find_sequences.py
#
# SEQUENCE_PAIRS is a list of (rgb_dir, depth_dir) tuples.
# Only pairs where BOTH folders exist AND share matching
# filenames are included.
SEQUENCE_PAIRS = auto_find_sequences(BASE_DIR, verbose=True)

IMG_SIZE    = (128, 416)   # (H, W)
BATCH_SIZE  = 2            # DPT is larger — use 2 if memory-tight, 4 with GPU
EPOCHS      = 40
MAX_DEPTH   = 80.0
LR          = 1e-4         # single LR for DPT (no pretrained backbone)
LR_WARMUP   = 5            # warmup epochs before cosine decay
VAL_SPLIT   = 0.1
SAVE_PATH   = "best_dpt_model.pth"    # ← DPT checkpoint

# DPT size config:
#   'tiny'  : embed=192, heads=3  — fastest, least accurate
#   'small' : embed=384, heads=6  — good balance  ← recommended
#   'base'  : embed=768, heads=12 — most accurate, needs GPU + more data
DPT_CONFIG  = 'small'
PATCH_SIZE  = 16


# ══════════════════════════════════════════════════════════
# DATASET  (identical to train_hybrid.py)
# ══════════════════════════════════════════════════════════

class KITTIDepthDataset(Dataset):
    """
    Loads matched (RGB, sparse-depth) pairs from KITTI.
    Augmentation applied jointly to RGB and depth to keep alignment.
    """
    ...
    def __getitem__(self, idx):
        ...
        return rgb_tensor, torch.from_numpy(depth).unsqueeze(0)


class AugmentedSubset(torch.utils.data.Dataset):
    """Wrapper to toggle augmentation on a random_split subset."""
...
# ══════════════════════════════════════════════════════════
# LOSS FUNCTIONS  (identical to train_hybrid.py)
# ══════════════════════════════════════════════════════════
...
class SSIMLoss(nn.Module):
    def __init__(self, window_size=11):
        super().__init__(); self.ws = window_size
...
if len(full_ds) == 0:
    raise RuntimeError("No samples found — check SEQUENCE_PAIRS paths.")
...
# ══════════════════════════════════════════════════════════
# MODEL — object created from depth_prediction_transformer.py
# ══════════════════════════════════════════════════════════

model = DepthPredictionTransformer(
    config     = DPT_CONFIG,   # 'tiny' | 'small' | 'base'
    patch_size = PATCH_SIZE,   # 16 — standard ViT patch size
    max_depth  = MAX_DEPTH,
).to(device)

criterion = CombinedLoss()

# Single optimizer for DPT (no pretrained backbone to protect)
optimizer = torch.optim.AdamW(
    model.parameters(), lr=LR, weight_decay=1e-4
)

# Linear warmup for LR_WARMUP epochs, then cosine decay
def lr_lambda(epoch):
    if epoch < LR_WARMUP:
...
print(f"\n{'='*60}")
print(f"  DPT-{DPT_CONFIG.upper()}  |  {EPOCHS} epochs  |  {device}")
print(f"  Loss = BerHu + SSIM×0.5 + Edge×0.5")
print(f"  Warmup: {LR_WARMUP} epochs  |  LR: {LR}")
...
axes[0].set_title(f'DPT-{DPT_CONFIG} — Total Loss'); axes[0].set_xlabel('Epoch')
...
plt.tight_layout()
plt.savefig('dpt_loss.png', dpi=150)
plt.show()
print("Saved → dpt_loss.png")
...
print(f"\n{'='*60}")
print(f"  DPT-{DPT_CONFIG} — Final Validation Metrics")
print(f"{'='*60}")
